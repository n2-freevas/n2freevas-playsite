...
export const getUnitsUsingFilter = async (
    ...
    try {
        // @ts-ignore
        let data: object[] = await Api.post(`${bs2ndGetUnitsUrl}?offset=${offset}&limit=${limit}`, {
            word,
            elem,
            tribe,
            gimmick
        ...
        // @ts-ignore
        return data.map((unit) => {
            return { ...unit, disable: false, indeck: false }
        })
    } catch (error) {
        throw error
    }
...
export const getAdvents = async (offset?: number, limit?: number): Promise<advent[]> => {
    try {
        // @ts-ignore
...
export const getAdventBanmen = async (advent_id: number): Promise<banmen[]> => {
    try {
        // @ts-ignore
        let data: object[] = await Api.get(`${bs2ndGetAdventBanmenUrl}?advent_id=${advent_id}`)
        // @ts-ignore
        return data
    } catch (error) {
        throw error
...
export const getSearchAnswer = async (banmen: string, target_kana: targetKana[], target_length: number[]): Promise<answerListModel> => {
    try {
        ...
        let data = await Api.post(`${bs2ndGetSearchAnswerUrl}`, {
            banmen,
            target_kana,
            target_word_length: target_length
        })
        // @ts-ignore
        return data
    } catch (error) {
