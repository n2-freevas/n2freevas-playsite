...
<div class='damage_kan' style="--id:{model.id};--x:{model.x}px; --y:{model.y}px; --size:{size}px; --scale:{scale};"
...
>
    ...
    <div class='effect-1 {effect_1_action ? "action-1":""}'></div>
    <div class='effect-2  {effect_2_action ? "action-2":""}'></div>
...
<style lang="scss">
    .damage_kan{
        ...
        @keyframes damekanEffect {
            0%{transform: scale(1); border-width: 5px; opacity: 1;}
            50%{transform: scale(1.2); border-width: 5px; opacity: 1;}
