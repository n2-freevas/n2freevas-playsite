...
<div class='loader' style='--loader_height:{`${large_petal_stroke * 2 + large_round_size}px`}; --color:{color};'>
    <div class='fulcrum'>
        ...
        {#each small_flowering_args as arg}
            <div class='petal {is_animate? "animate" : ""}' style='--size:{`${small_round_size}px`}; --move_x:{arg.flower_petal_x}; --move_y:{arg.flower_petal_y}; --delay:{`${small_stroke_delay}s`}'></div>  
        {/each}
        </div>
        <div class='petals {is_animate? "animate" : ""}' style='--size:{`${large_round_size}px`}; --speed:{large_round_spin_speed}'>
            ...
            <div class='petal {is_animate? "animate" : ""}' style='--size:{`${large_round_size}px`}; --move_x: {arg.flower_petal_x}; --move_y: {arg.flower_petal_y}; --delay:{`${large_stroke_delay}s`}'></div>  
            {/each}
        </div>
    </div>
</div>
        ...
        .fulcrum{
            ...
            .petal{
                ...
                @keyframes flowering{
                    ...
                    40%{transform: translate(var(--move_x), var(--move_y))}
