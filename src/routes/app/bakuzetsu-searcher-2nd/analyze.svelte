...
import { adventStore, adventFilterConditionStore, adventElemStore,
    adventTargetStore, adventBanmenStore, deckStore, mojiLengthConfigStore } from "$lib/store/app/bs2ndStore"
...
<article class='com_scroll-y'>
    <section id='advent-list-box'>
        ...
        <div id='advent-list' class='com_scroll-x'>
            ...
            {:else}
                ...
                <Advent advent={as} full={true} on:pickup={pickUpAdventHandler} on:pickdown={pickDownAdventHandler}></Advent>
                <InfiniteScroll threshold={100} on:loadMore={() => {scrollAdventsListHandler()}} />
                {/each}
            {/if}
        </div>
    </section>
    <section id='elem-filter-box'>
        {#each $adventElemStore as es, i}
            <div class='elem {ElemEngDict[es.elem]} {es.active ? "active":""}'
    ...
    <section id='advent-detail-box'>
        ...
        {:else}
            <div id='advent-overview'>
                ...
                <Advent advent={$adventTargetStore}></Advent>
            ...
            <div id='banmen-list'>
                ...
                {#each $adventBanmenStore as abs}
                    <div class='banmen {abs.active ? 'active' : ''}' on:click={()=>{
            ...
            <div id='answers-box' class={isActiveAnswerBox ? 'active': ''}>
                <div id='answer-box-close-button'></div>
                <div id='answer-box-close-button-tap-area' on:click={()=>{isActiveAnswerBox = false}}>
        ...
        #advent-detail-box{
            ...
            #banmen-list{
                ...
                .banmen{
                    ...
                    padding:0 0 5px 0; 
                    margin: 0 0 15px 0;
                    img{width:8px; height: 15px; margin:0 20px 0 0;transition: .5s;}
                    &.active{
                        color:var(--active-yellow);
                        img{
